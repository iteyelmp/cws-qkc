...
export const getRawHex = (transaction: any): Array<Buffer> => {
    ...
    raw[6] = Buffer.from(handleHex(transaction['networkId']), 'hex');
    raw[7] = Buffer.from(handleHex(transaction['fromFullShardKey']), 'hex');
    raw[8] = Buffer.from(handleHex(transaction['toFullShardKey']), 'hex');
    raw[9] = Buffer.from(handleHex(transaction['gasTokenId']), 'hex');
    raw[10] = Buffer.from(handleHex(transaction['transferTokenId']), 'hex');
